...
-- To create a new picker in Telescope for NeoVim, you can define a new `picker` module that lists the desired options.
function M.chatgpt_run(opts)
    -- Define the options to display in the picker
    local options = {
        "grammar_correction",
        "translate",
        "keywords",
        ...
        "fix_bugs",
        "explain_code",
        "roxygen_editlain_code",
        "roxygen_edit",
    }

    -- Call Telescope's `pickers.new` function to create the picker
    return require("telescope.pickers")
        .new(opts, {
            prompt_title = "ChatGPT Run",
            ...
            attach_mappings = function(prompt_bufnr)
                actions.select_default:replace(function()
                    ...
                    local cmd = string.format([[:ChatGPTRun %s ]], val)
                    vim.cmd(cmd)
...
M.lazy = function()
    require("telescope").extensions.lazy.lazy(M.get_theme())
end
...
M.bibtex = function()
    require("telescope").extensions.bibtex.bibtex(M.get_theme())
end
